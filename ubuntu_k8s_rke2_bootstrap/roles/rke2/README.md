...
Installs RKE2 on Ubuntu via the official `get.rke2.io` script: a `server`
(control-plane) or an `agent` (worker). Writes
`/etc/rancher/rke2/config.yaml`, opens the RKE2 ports in `ufw`, then enables
`rke2-server` / `rke2-agent` via systemd.
...
| `rke2_firewall_manage` | `true` | Open RKE2 ufw ports per node type |
