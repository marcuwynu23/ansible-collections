# Ansible Collection - marcuwynu23.ubuntu_k8s_rke2_bootstrap

Bootstrap a Kubernetes cluster with [RKE2](https://docs.rke2.io/) on Ubuntu
virtual machines: OS preparation, optional standalone containerd, and RKE2
server (control-plane) / agent (worker) installation.

Ubuntu counterpart of `marcuwynu23.fedora_k8s_rke2_bootstrap` — same roles
...
| Path | Purpose |
|------|---------|
| `roles/pre` | Ubuntu node prep: apt upgrade, base packages, swap off, kernel modules, sysctl, ufw |
...
| SELinux mode set to `permissive` | Not needed — Ubuntu runs AppArmor, RKE2 works with defaults |
