...
Installs K3s on Ubuntu via the official `get.k3s.io` script: a `server`
(control-plane) or an `agent` (worker). Writes
`/etc/rancher/k3s/config.yaml` **before** installing (the script starts the
service immediately), opens the K3s ports in `ufw`, then enables `k3s` /
`k3s-agent` via systemd.
...
| `k3s_firewall_manage` | `true` | Open K3s ufw ports per node type |
