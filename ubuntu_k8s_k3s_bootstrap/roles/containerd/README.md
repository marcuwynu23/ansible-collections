...
Installs and configures standalone containerd on Ubuntu (`apt`,
`/etc/containerd/config.toml` with `SystemdCgroup = true`, systemd service).

> **Note:** K3s ships and manages its own embedded containerd, so the
> collection playbooks set `containerd_enabled: false` by default. Enable
> this role only if you also want a system-wide containerd next to K3s.
...
| `containerd_enabled` | `true` | `false` skips everything (used by the K3s playbooks) |
