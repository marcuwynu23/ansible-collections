...
Ubuntu OS preparation for K3s Kubernetes nodes. Replaces manual setup:
hostname, system updates, swap, kernel modules, sysctl and firewall.
Uses `apt`/`ufw` (no `dnf`/`firewalld`). Ubuntu runs AppArmor (K3s works
...
- Applies sysctl (`ip_forward`, bridge-nf-call) via `/etc/sysctl.d/99-k8s-k3s.conf`.
- Allows SSH, opens K3s firewall ports (6443 API, 10250 kubelet,
  flannel/wireguard, NodePorts, etcd) and enables `ufw`.
...
| `pre_firewall_manage` | `true` | Manage ufw + K3s ports |
...
| `pre_sysctl_params` | (dict) | Sysctl keys written to `99-k8s-k3s.conf` |
