# Ansible Collection - marcuwynu23.ubuntu_k8s_k3s_bootstrap

Bootstrap a Kubernetes cluster with [K3s](https://k3s.io/) on Ubuntu virtual
machines: OS preparation, optional standalone containerd, and K3s
server (control-plane) / agent (worker) installation.

Ubuntu counterpart of `marcuwynu23.fedora_k8s_k3s_bootstrap` — same roles
...
| SELinux mode set to `permissive` | Not needed — Ubuntu runs AppArmor, K3s works with defaults |
